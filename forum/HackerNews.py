                ...
                if r.text.find("No such user.") != 0:
                    ...
                    try:
                        user_creation_date = str(
                            ...
                            "table") else None
                        user_karma = str(
                            soup.find_all("table")[2].find_all("td")[5].get_text()).strip() if soup.find_all(
                            "table") else None
                        ...
                        self.log.error(f'Error en la búsqueda de información de {username} en hackernews')

                    # Append the account to the accounts table
                    hackernews_usernames["accounts"].append(account)

            except requests.ConnectionError:
                self.log.error('Error al realizar la petición a hackernews')
